...
\begin{thm}[Asymptotic Shrinkage for Variance Ratio of Maximum Sharpe Ratio
...
\begin{itemize}
	...
	\item Monotonicity constraint for $z^{(k)}$, ie. solve the constrained least
	squares problem:
	\begin{equation*}
	  \begin{aligned}
	    & \underset{z}{\text{minimize}}
	  ...
	  \end{aligned}
	\end{equation*}
	where $G\in \mathbb{R}^{(N-1)\times N}$ is the monotonicity contraint such
	that $G_{ii} = 1$ and $G_{i,(i+1)} = -1$ and 0 otherwise.  An extra row can be
	added to impose non-negativity by requiring $z_N \geq 0$.

	\item $L^2$ regularization on $z^{(k)}$:
	\begin{equation*}
	  \begin{aligned}
	    & \underset{z}{\text{minimize}}
	    & & \| C^{(k)} A^{(k)} z - \alpha^{(k)} \|_2^2 + \gamma \|z\|_2^2,
	  \end{aligned}
	\end{equation*}
	...
	$$
		C^{(k)} = {U^{(k)}}^T S_k U^{(k)},
	$$
	where $S_k$ is some covariance estimator replacement for $x_k x_k^T$.


\end{itemize}

\item \emph{MinVar Leave-One-Out/$K$-Fold Joint Cross-Validation with Isotonic
Regression}

See Algorithm \ref{alg:minvar_loo_joint_cv} in the appendix for a detailed
implementation.  See Algorithm \ref{alg:minvar_kfold_joint_cv} in the
appendix for the $K$-Fold variant.
...
\begin{algorithm}
	...
	\Procedure{MinVar-LOO-CV}{$X$}
		...
		\State $\hat{d}^* \gets \mathrm{Isotonic}(\bar{d}).$ 
			\Comment{Isotonic Regression applied to $\bar{d}$}

		\State $\hat{\Sigma}_N \gets UDU^T, \quad 
			\mbox{where}\quad D = \mathrm{Diag}(\hat{d}^*_1, \ldots, \hat{d}^*_N)$

		\State \textbf{return} $\hat{\Sigma}_N$ 
				\Comment{The MinVar LOO-CV non-linear shrinkage estimator for
				$\Sigma_N$.}
	\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
	...
	\Procedure{MinVar-$K$-Fold-CV}{$X$}
		...
		\State \textbf{return} $\hat{\Sigma}_N$ 
				\Comment{The MinVar $K$-Fold-CV non-linear shrinkage estimator for
				$\Sigma_N$.}
	\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
	...
	\Require $X = [x_1, \ldots, x_T]^T \in \mathbb{R}^{T\times N}$
	\Require $\Sigma_N = \mathrm{Cov}(X)$
	\Procedure{MinVar-LOO-Joint-CV}{$X$}
		...
		\State \textbf{return} $\hat{\Sigma}_N$ 
				\Comment{The MinVar LOO-Joint-CV non-linear shrinkage estimator for
				$\Sigma_N$.}
	\EndProcedure
	\end{algorithmic}
\end{algorithm}
...
\begin{algorithm}
	...
	\Procedure{MinVar-$K$-Fold-CV}{$X$}
		...
		\State \textbf{return} $\hat{\Sigma}_N$ 
				\Comment{The MinVar $K$-Fold-Joint-CV non-linear shrinkage estimator for
				$\Sigma_N$.}
	\EndProcedure
	\end{algorithmic}
\end{algorithm}
